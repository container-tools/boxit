...
import (
	...
	util "github.com/container-tools/boxit/server/pkg/util"
...
func GenerateProjectStructure(context Context) error {
	...
	for k, v := range context.AdditionalEntries {
		...
		if len(bytes) > 0 {
			log.Printf("write entry: %s (%d bytes)", k, len(bytes))
...
func Run(ctx Context) error {
	...
	log.Printf("executing: %s, timeout is %s", strings.Join(cmd.Args, " "), timeout.String())
